...
def test_album_manifest_promotes_gold_v6_to_protected_incumbent() -> None:
    ...
    # The accepted master is the descendant of this lineage, not a replacement for
    # it: acceptance must not release the protected incumbent, rewrite its review
    # receipt, or reopen recovery.
    assert track["status"]["album_master"] == "accepted"
    assert track["status"]["human_acceptance"] is True
    assert track["status"]["evidence_state"].startswith("accepted_album_master")
