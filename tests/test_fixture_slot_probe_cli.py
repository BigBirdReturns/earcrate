...
def _atom(source, atom, role, bpm, key):
    ear = {"foreground": "VOX_HOOK", "floor": "DRUM_BREAK", "bass": "BASS_RIFF", "spark": "DROP_HIT"}[role]
    render = {"foreground": "vocal", "floor": "drum_anchor", "bass": "bass", "spark": "texture"}[role]
    return {
        "id": atom, "atom_id": atom, "source_track_key": source,
        "ear_role": ear, "render_role": render, "role": render,
        "bpm": bpm, "key_root": key, "bars": 4,
        "start_s": 0.0, "end_s": 10.0,
        "source_audio_sha256": source + "-pcm", "score": 0.8, "hook_score": 0.8,
    }


class _Core:
    def __init__(self, pool):
        self.pool, self.raw_calls, self.exact_calls = list(pool), 0, 0
    def approved_atom_pool(self, _profile):
        return list(self.pool)
    def taste_feasible_pool(self, pool, bpm, key, _params):
        out = [dict(item) for item in pool if abs(float(item["bpm"]) - float(bpm)) < 1e-9 and int(item["key_root"]) % 12 == int(key) % 12]
        return out, {"have": {"sources": len({item["source_track_key"] for item in out})}}
    ...
    def _ordinary_compose_taste_arrangement(self, pool, params, seed):
        ...
        return {
            "bpm": deck["render_bpm"], "target_key": deck["target_key"],
            "seed": seed, "params": dict(params), "dj_compiler": {"version": "probe"},
            "sections": [
                {"bar_start": index * 4, "bars": 4, "type": "sustain",
                 "layers": [{"role": item["render_role"], "source_track_key": item["source_track_key"], "atom_id": item["atom_id"]}]}
...
def _probe_fixture():
    ...
    for index, bpm, key in ((0, 120.0, 0), (1, 130.0, 5)):
        ...
        islands.append({
            "island_id": f"island-{index}", "deck_id": f"deck-{index}",
            "target_bpm": bpm, "target_key": key,
            "capacity_s": 40.0, "allocated_duration_s": 40.0,
            "start_s": cursor, "end_s": cursor + 40.0,
            "source_include_ids": sources,
            "required_roles": ["foreground", "floor", "bass", "spark"],
        })
        cursor += 40.0
    candidate = {
        "kind": "earcrate_fixture_candidate", "profile": "girl_talk_v1",
        "persona": "remix_prettylights_v1", "phrase_playback_law": "proof001_phrase_law",
        "source_pool_sha256": source_pool_identity(pool), "source_exclude_ids": [],
        "transform_policy": {"unchanged": True, "stretch_budget": 8.0, "pitch_shift_budget": 2},
        "turnover_policy": {"unchanged": True},
        "transition": {"technique": "equal_power", "phrase_boundary_required": True},
        "duration_s": 55.0, "phrase_bars": 4, "seed": 7,
...
def test_slot_probe_uses_the_ordinary_composer_and_never_publishes():
    ...
    assert core.raw_calls == 2 and core.exact_calls == 0
    ...
    assert "source_track_key" not in text and "atom_id" not in text
...
def _matrix_and_candidate():
    from test_fixture_slot_qualification import _candidate, _census, _matrix, _repair_fixture
...
def test_slot_qualification_cli_refuses_alias_and_writes_a_bound_candidate(tmp_path):
    ...
    matrix_path, candidate_path, census_path = tmp_path / "matrix.json", tmp_path / "candidate.json", tmp_path / "census.json"
    matrix_path.write_text(json.dumps(matrix), encoding="utf-8")
    candidate_path.write_text(json.dumps(candidate), encoding="utf-8")
    census_path.write_text(json.dumps(census), encoding="utf-8")
    before = candidate_path.read_bytes()
    assert cli.main([
        "qualify", str(matrix_path), str(candidate_path), str(census_path),
        "--candidate-out", str(candidate_path), "--receipt", str(tmp_path / "receipt.json")
    ]) == 2
    assert candidate_path.read_bytes() == before
    output, receipt = tmp_path / "qualified.json", tmp_path / "receipt.json"
    assert cli.main([
        "qualify", str(matrix_path), str(candidate_path), str(census_path),
        "--candidate-out", str(output), "--receipt", str(receipt),
        "--max-source-events", "3"
    ]) == 0
    ...
    for name, path in (("survival_matrix", matrix_path), ("candidate", candidate_path), ("slot_census", census_path)):
        assert report["input_files"][name]["file_sha256"] == hashlib.sha256(path.read_bytes()).hexdigest()
