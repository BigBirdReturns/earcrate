...
def test_absent_from_this_workstation_is_not_the_same_claim_as_unavailable():
    """The correction this branch carries, kept as a gate rather than a memory.

    Three derived objects are genuinely gone: their identities are known and their
    bytes exist nowhere. The source score is a different case -- absent from the
    directories searched here, but held in project attachment custody, so recoverable.
    Collapsing those two into one word would have closed a search that should stay open.
    """
    manifest = json.loads(
    ...
    assert legacy["classification"] == "partially_recovered_external_evidence"
    assert legacy["search_status"] == "closed_for_legacy_derived_objects"
    assert "does not authorize inventing bytes under the old digests" in legacy["why"]
    assert "absent from this workstation" in legacy["correction"].lower() or         "absent from the searched directories" in legacy["correction"].lower()
    assert len(legacy["objects"]) == 4

    derived = [row for row in legacy["objects"] if not row["artifact"].endswith(".pdf")]
    assert len(derived) == 3
    for entry in derived:
        assert entry["identity_known"] is True
        assert entry["bytes_available"] is False
        assert entry["status"] == "unavailable_external_evidence"
        assert len(entry["container_sha256"]) == 64

    score = next(row for row in legacy["objects"] if row["artifact"].endswith(".pdf"))
    assert score["bytes_available"] is True
    assert score["status"] == "recoverable_external_evidence"
    assert score["availability"] == "project_attachment"
    assert score["local_materialization"] == "pending"
    assert score["sha256_verification"] == "pending"
    assert score["expected_sha256"] == (
        "e029e1a3030800d7fb04c9f5163acb9270579a571d57f63eb63787df692d5845")
    # Until a copy is materialized and hashed, no copy may be called the source object.
    assert "container_sha256" not in score,         "an unverified copy must not carry a settled container identity"
