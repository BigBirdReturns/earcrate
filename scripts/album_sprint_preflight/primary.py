...
def children(track_id: str, spec: Mapping[str, Any], bindings: Mapping[str, Any]) -> dict[str, Any]:
    ...
    required = list(spec.get("required_bindings") or [])
    score_pack = list(spec.get("score_pack_bindings") or required)
    performance = list(spec.get("performance_bindings") or [])
    convergence = list(spec.get("convergence_bindings") or [])
    all_missing = missing(bindings, required)
    score_missing = missing(bindings, score_pack)
    performance_missing = missing(bindings, performance)
    convergence_missing = missing(bindings, convergence)
    result["binding_contract_ready"] = not all_missing
    ...
    result["observations"] = {
        "required_exact_artifact_ids": score_pack,
        "required_performance_binding_ids": performance,
        "required_convergence_binding_ids": convergence,
    ...
    if score_missing:
        result["blockers"].append(blocker(
            ...
            missing_artifact_ids=score_missing,
        ))
    if performance_missing:
        result["blockers"].append(blocker(
            "blocked_performance_material", "binding_contract",
            "The approved rack library required for an audible full-performance realization is not bound.",
            missing_binding_ids=performance_missing,
        ))
    if convergence_missing:
        result["blockers"].append(blocker(
            "blocked_exact_source", "binding_contract",
            "The exact reference recording required for score/audio convergence is not bound.",
            missing_binding_ids=convergence_missing,
        ))
    result["blockers"].extend([
        blocker(
            "blocked_representative_invocation", "execution_evidence",
            "No current exact-head invocation receipt proves the complete bound Children adapter path."
        ),
        blocker(
...
def flim(track_id: str, spec: Mapping[str, Any], bindings: Mapping[str, Any]) -> dict[str, Any]:
    result = base_result(track_id, str(spec["adapter"]))
    ...
    result["observations"] = {
        ...
        "representative_invocation_scope": "symbolic_report_only",
    }
    if absent:
        result["blockers"].append(blocker(
            "blocked_exact_source", "binding_contract",
            "The exact Bad Plus performance and approved trio racks are not both bound.",
            missing_binding_ids=absent,
        ))
    result["blockers"].extend([
        blocker(
